...
void update_bit_string(struct Node *cur, char *bitStringArray[], char *currentString){
	...
	if(cur->type == 0){
		...
		update_bit_string(cur->left, bitStringArray, currentString);
		
		currentString[strlen(currentString)-1] = '1';
		
		update_bit_string(cur->right, bitStringArray, currentString);
...
int main(int argc, char*argv[]){
	...
	update_bit_string(head, bitStringArray, currentString);
	
	//get the EOF string
	FILE * f= fopen("workHorse.c", "a");
	printf("\n%s", bitStringArray[255] );
	fprintf(f,"\n%s", bitStringArray[255] );
	fclose(f);
	...
	while(c != EOF) {
		...
		strcat(resultString, bitStringArray[(int)c]);
		c=fgetc(fp);
	}
